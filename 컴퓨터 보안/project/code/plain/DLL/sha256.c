...
#define BIG_B2D(B, D)		D = *(ULONG*)(B)
#define BIG_D2B(D, B)		*(ULONG*)(B) = (ULONG)(D)
#define LITTLE_B2D(B, D)	D = ENDIAN_REVERSE_ULONG(*(ULONG*)(B))
#define LITTLE_D2B(D, B)	*(ULONG*)(B) = ENDIAN_REVERSE_ULONG(D)
#elif defined(LITTLE_ENDIAN)
#define BIG_B2D(B, D)		D = ENDIAN_REVERSE_ULONG(*(ULONG*)(B))
#define BIG_D2B(D, B)		*(ULONG*)(B) = ENDIAN_REVERSE_ULONG(D)
#define LITTLE_B2D(B, D)	D = *(ULONG*)(B)
#define LITTLE_D2B(D, B)	*(ULONG*)(B) = (ULONG)(D)
#else
...
// o 출력				: 
//*********************************************************************************************************************************
void SHA256_Transform(ULONG* Message, ULONG* ChainVar)
{
...
void SHA256_Process(OUT SHA256_INFO *Info, IN const BYTE *pszMessage, IN UINT uDataLen)
{
	...
	while (uDataLen >= SHA256_DIGEST_BLOCKLEN)
	{
		...
		SHA256_Transform((ULONG*)Info->szBuffer, (ULONG*)Info->uChainVar);
...
void SHA256_Close(OUT SHA256_INFO *Info, IN BYTE *pszDigest)
{
	...
	((ULONG*)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 2] = Info->uHighLength;
	((ULONG*)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 1] = Info->uLowLength;
