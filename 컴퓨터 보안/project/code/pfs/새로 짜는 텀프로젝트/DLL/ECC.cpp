...
int mode(int num, int p, int flag);
int GetM(point p1, point p2, int a, int p);
void Set_Color_Console(WORD color);

void Set_ECC(int x_value, int y_value, int mod_value, point x)
{
...
point Perfect_P(point x, int mul, int P)
{
	point s;
	
	if (P == PUBLIC_POINT)
	{
		s = ECC(x_v, y_v, m_v, set, mul);
		printf("(%d,%d) * %d = (%d,%d) \n", set.x, set.y, mul, s.x, s.y);
		return s;
	}
	else if (P == PRIVATE_POINT)
	{
		s = ECC(x_v, y_v, m_v, x, mul);
		printf("(%d,%d) * %d = (%d,%d) \n", x.x, x.y, mul, s.x, s.y);
		return s;
	}

	//return s = ECC(11, 19, 167, x, mul);
