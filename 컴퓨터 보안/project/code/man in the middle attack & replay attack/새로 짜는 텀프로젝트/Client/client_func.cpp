...
void Login(SOCKET sock, int select, INFO* information)
{
	...
	SOCKET hack = Make_Client(10000, "127.0.0.1");
	if (F_Recv(hack, buf) == -1) return;
	int nonce = atoi(buf);
	sprintf(buf, "Bob");
	A5_1(buf, Key);
	F_Send(hack, buf);
	sprintf(buf, "IamBob,%d", nonce);
	A5_1(buf, Key);
	F_Send(hack, buf);
	closesocket(hack);
	...
	if (F_Recv(sock, buf) == -1) return;
	printf("\nNonce_S : %s \n", buf);
	sprintf(information->pw, "%s,%s", information->pw, buf);
	printf("보낼 데이터 : %s \n", information->pw);
	A5_1(information->id, Key);
	F_Send(sock, information->id);
	A5_1(information->pw, Key);
	...
	if (atoi(buf) == -1)
	{
		printf("NONCE 값이 다릅니다! 3초 후 프로그램을 종료합니다.\n");
		Sleep(3000);
		exit(1);
	}
	A5_1(buf, Key);
